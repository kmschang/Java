...
First defined all the variables needed, to solve the board. Then I made the main
method to solve it. Checks for when zeros is larger than 4 and stops. I made two
function for solving with one missing, one horizontal and the other vertical. I
made one method for when there are two missing, and it references the two other
one missing ones. The last function is based off what Hobbs put into blackboard.
I watched his video a few times and I think I got it the way he wanted.
...
public class SchangKyleProj1 {
  ...
  // row to keep track of where we are when we are going through the board
  static int row = 0;
  ...
  // main function. Tried to minimize it as much as I could and make most of it
  // modular
  public static void main(String[] args) {
    // does while zeros is less than 4. No need to check all the way up to 81
    // because after 3, we won't need to solve it.
    do {
      readBoard();
  ...
  public static void readBoard() {
    ...
    for (row = 0; row < 9; ++row) {
      for (col = 0; col < 9; ++col) {
        ...
        // checks to see if the one it just inputted was a zero, if it was, it
  ...
  // main function for solving the board. Check to see how many zeros there are
  // and then points to the necessary function
  public static void solveBoard() {
    ...
    if (zeros == 2) {
      ...
      answers[1][0] = location[1][0];
      answers[1][1] = location[1][1];
      answers[1][2] = typeTwo()[1];
    }
    // three zeros missing, solves with type three zero
    if (zeros == 3) {
      int oneX = location[0][0];
      int oneY = location[0][1];
      int twoX = location[1][0];
      int twoY = location[1][1];
      int threeX = location[2][0];
      int threeY = location[2][1];
      int[] and = typeThree();
      System.out.print("(" + oneX + "," + oneY + "," + and[0] + ")");
      System.out.print(" (" + twoX + "," + twoY + "," + and[1] + ")");
      System.out.println(" (" + threeX + "," + threeY + "," + and[2] + ")");
    }
  }
  ...
  // method for solving two missing, uses typeOne in typeTwo to solve them
  // individually
  ...
  public static int[] typeThree() {
    // initializing new variables to keep track of all the arrays to find the
    // zeros and values
    ...
    // we found the first one with the whichbox and threebycounts but now it is
    // the time to find the other two to find the other two, we just used the
    // typeTwo function
