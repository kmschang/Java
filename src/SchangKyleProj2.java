...
Started off by making the person class with a variable name. I made instances so
that you could input the name into the class. I then made the methods to set the
name, get the name, reset the person, write the output and see if there is an
overlap

Then I made the student class which extends the person class. This class has a
new variable student number. I made methods to set the number, get the number,
reset the person, write an output and check or overlap with people.

Then I made the undergraduate class which extends student with the student level
variable. I made methods to set the level, get the level, reset the person,
write an output and check or overlap with people.

Then I made the graduate class which also extends student with a student level
variable as well. I made methods to set the level, get the level, reset the
person, write an output and check or overlap with people.

Then I made the employee class which extends person. This class has a new
variable employee ID. I made methods to set the ID, get the ID, reset the
person, write the output, and check for overlap.

Then I amd the Faculty class which extends employee class. This class has two
new variables, department and title. I made methods to set and get both of the
new variables along with output, reset the person, anc check for overlapping.

Then I made the last class Staff which extends employee. This class only has one
new variable pay grade. I made methods to set pay grade, get pay grade, reset
the person, write output, and check for overlap.

Then I made test cases to make sure all the printing was working, and it was
passing all the variables correctly.
...
public class SchangKyleProj2 {
  ...
  static class Employee extends Person {
    ...
    // set employee id
    void setEmployeeID(int newEmployeeID) { employeeID = newEmployeeID; }
  ...
  static class Staff extends Employee {
    ...
    // new department and pay grade variable
    String department;
    int payGrade;
    ...
    // set pay grade
    void setPayGrade(int newPayGrade) { payGrade = newPayGrade; }

    // getting department
    String getDepartment() { return department; }

    // getting pay grade
    int getPayGrade() { return payGrade; }
